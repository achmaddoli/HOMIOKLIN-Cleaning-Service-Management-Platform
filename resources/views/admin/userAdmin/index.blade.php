            ...
            <div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 overflow-hidden">
                <div class="p-6 md:p-8">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                        <div>
                            <h3 class="text-lg font-bold text-slate-800">Daftar Administrator</h3>
                            <p class="text-sm text-slate-500">Kelola akun staf yang memiliki hak akses sistem pendaftaran layanan.</p>
                        </div>
                        <a href="{{ route('user.admin.create') }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-cyan-500 text-white font-bold rounded-xl hover:bg-cyan-600 hover:shadow-lg hover:shadow-cyan-500/30 transition-all duration-300">
                        ...
                        <table id="example" class="w-full text-left border-collapse" style="width:100%;">
                            <thead>
                                <tr class="bg-slate-50 text-slate-500 text-sm uppercase tracking-wider border-b border-gray-100">
                                    <th class="px-6 py-4 font-bold rounded-tl-xl w-1/4" data-priority="1">Profil Admin</th>
                            ...
                            <tbody class="text-slate-700 text-sm">
                                ...
                                <tr class="border-b border-gray-50 hover:bg-slate-50/50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 text-slate-600 font-medium">{{ $admin->email }}</td>
                                    <td class="px-6 py-4 text-slate-600">{{ $admin->hp }}</td>
                                    <td class="px-6 py-4 text-slate-500 min-w-[150px] truncate max-w-[200px]" title="{{ $admin->alamat }}">{{ $admin->alamat }}</td>
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center items-center gap-2">
                                            <a href="{{ route('user.admin.edit', $admin->id) }}" class="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-orange-50 text-orange-500 hover:bg-orange-500 hover:text-white transition-colors duration-200 tooltip" title="Edit Data">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                            </a>
                                            <button onclick="confirmDelete({{ $admin->id }})" class="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-colors duration-200 tooltip" title="Hapus Data">
