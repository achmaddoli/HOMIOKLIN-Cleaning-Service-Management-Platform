<x-app-layout>
    ...
    <style>
        ...
        /* Tambahan agar tabel tidak terpotong (Scrollable horizontal) */
    ...
    <div class="bg-slate-50 min-h-screen py-10 font-sans selection:bg-cyan-200 selection:text-cyan-900">
        <div class="max-w-[95rem] mx-auto sm:px-6 lg:px-8"> <div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 overflow-hidden">
                        ...
                        <table id="example" class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider border-b border-gray-100">
                                    ...
                                    <th class="px-4 py-4 font-bold" data-priority="8">Catatan</th>
                                    <th class="px-4 py-4 font-bold text-center" data-priority="9">Status</th>
                                    <th class="px-4 py-4 font-bold text-center rounded-tr-xl" data-priority="10">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="text-slate-700 text-sm">
                                ...
                                <tr class="border-b border-gray-50 hover:bg-slate-50/50 transition-colors duration-200 align-middle">
                                    ...
                                    <td class="px-4 py-4 text-slate-500 italic text-xs min-w-[100px] truncate max-w-[150px]" title="{{ $booking->notes }}">{{ $booking->notes ?? '-' }}</td>
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex justify-center items-center gap-1.5">
                                            @if ($booking->status == 'Menunggu')
                                                <button onclick="confirmReceipt({{ $booking->id }})" class="w-8 h-8 rounded-lg bg-green-50 text-green-600 hover:bg-green-500 hover:text-white flex items-center justify-center transition-colors tooltip" title="Terima">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                                                </button>
                                                <form id="accept-form-{{ $booking->id }}" action="{{ route('booking.terima', $booking->id) }}" method="POST" style="display: none;">@csrf @method('PUT')</form>

                                                <button onclick="confirmReject({{ $booking->id }})" class="w-8 h-8 rounded-lg bg-red-50 text-red-600 hover:bg-red-500 hover:text-white flex items-center justify-center transition-colors tooltip" title="Tolak">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>
                                                </button>
                                                <form id="reject-form-{{ $booking->id }}" action="{{ route('booking.tolak', $booking->id) }}" method="POST" style="display: none;">@csrf @method('PUT')</form>

                                                <a href="{{ route('booking.show', $booking->id) }}" class="w-8 h-8 rounded-lg bg-cyan-50 text-cyan-600 hover:bg-cyan-500 hover:text-white flex items-center justify-center transition-colors tooltip" title="Detail">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                                </a>

                                            @elseif ($booking->status == 'Selesai')
                                                <a href="{{ route('payment.create', $booking->id) }}" class="w-8 h-8 rounded-lg bg-orange-50 text-orange-500 hover:bg-orange-500 hover:text-white flex items-center justify-center transition-colors tooltip" title="Proses Bayar">
