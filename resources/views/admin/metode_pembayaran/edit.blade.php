            ...
            <div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 overflow-hidden">
                <div class="p-8 md:p-12">
                    ...
                    <div class="mb-8 border-b border-gray-100 pb-6">
                        <h3 class="text-xl font-bold text-slate-800">Perbarui Detail Pembayaran</h3>
                        <p class="text-slate-500 mt-1">Ubah nama metode pembayaran untuk ID <span class="font-bold text-slate-700">#{{ $type->id }}</span>.</p>
                    </div>

                    <form action="{{ route('type.update', $type->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        
                        <div>
                            <label for="name" class="block text-sm font-bold text-slate-700 mb-2">Nama Metode Pembayaran</label>
                            <input type="text" name="name" id="name" value="{{ $type->name }}" class="block w-full px-4 py-3.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-colors text-slate-800" required>
                        </div>
